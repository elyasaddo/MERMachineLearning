...
class Markdown implements MarkdownInterface {
	...
	protected function _doAnchors_reference_callback($matches) {
		...
		if (isset($this->urls[$link_id])) {
			...
			$result = "<a href=\"".$GLOBALS['base']."$url\"";
...
abstract class _MarkdownExtra_TmpImpl extends \Markdown\Markdown {
	...
	protected function _doImages_inline_callback($matches) {
		...
		$alt_text = $this->encodeAttribute($alt_text);
		$url = $this->encodeAttribute($url);
		$result = "<img src=\"".$GLOBALS['base']."$url\" alt=\"$alt_text\"";
		if (isset($title)) {
			$title = $this->encodeAttribute($title);
			$result .=  " title=\"$title\""; # $title already quoted
		}
	...
	#
		...
		if (!empty($this->footnotes_ordered)) {
			...
			while (!empty($this->footnotes_ordered)) {
				...
				$backlink = "<a href=\"#fnref:$note_id\"$attr><i class=\"icon-up-right small\"></i></a>";
				for ($ref_num = 2; $ref_num <= $ref_count; ++$ref_num) {
					$backlink .= " <a href=\"#fnref$ref_num:$note_id\"$attr><i class=\"icon-up-right small\"></i></a>";
				}
